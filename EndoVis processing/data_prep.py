...
if __name__ == '__main__':
    for instrument_index in range(1, 9):
        ...
        for file_name in tqdm(list((train_path / instrument_folder / 'left_frames').glob('*'))):
            ...
            cv2.imwrite(str(cropped_train_path / instrument_folder / 'images' / (str(instrument_index) + (file_name.stem + '.jpg'))), img,
            ...
            #cv2.imwrite(str(binary_mask_folder / (str(instrument_index) + file_name.name)), mask_binary)
            cv2.imwrite(str(parts_mask_folder / (str(instrument_index) + file_name.name)), mask_parts)
            #cv2.imwrite(str(instrument_mask_folder / (str(instrument_index) + file_name.name)), mask_instruments)
